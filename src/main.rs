...
fn run_app(shell_mode: bool) -> Result<()> {
    let recognizer =
        InkRecognizerService::new().context("failed to initialize Windows Ink recognizer")?;
    ...
    run_interactive_loop(handwriting_manager.as_ref(), &recognizer)
}

fn run_interactive_loop(
    handwriting_manager: Option<&Rc<RefCell<HandwritingManager>>>,
    recognizer: &InkRecognizerService,
) -> Result<()> {
    ...
    loop {
        ...
        unsafe {
            while PeekMessageW(&mut msg, None, 0, 0, PM_REMOVE).as_bool() {
                ...
                if msg.message == WM_HOTKEY && msg.wParam.0 == HOTKEY_CUSTOM as usize {
                    floating_picker.hide();
                    if let Err(e) = run_custom_session(handwriting_manager, recognizer, None) {
        ...
        if let Some(target) = floating_picker.take_confirmed() {
            if let Err(e) = run_custom_session(handwriting_manager, recognizer, Some(target)) {
                warn!(?e, "handwriting session failed");
            }
            focus_watch.clear_trigger();
        }
